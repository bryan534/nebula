...
If `CONFIGURE_PAGE_PASSWORD` is configured, Nebula shows a login form first.
...
- Remote/VPS/domain deployments should expose Nebula through HTTPS on a reverse proxy before installing in Stremio.
- If you run Nebula locally on the same machine as Stremio Desktop, `http://127.0.0.1:8000` or `http://localhost:8000` can work.

## Step 4: Verify Streams

After installing in Stremio, open a movie or episode.

Nebula serves streams from:
...
- If you configured torrent-only mode and `DISABLE_TORRENT_STREAMS=True`, Nebula returns a placeholder stream message by design.
